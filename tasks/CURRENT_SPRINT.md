...
| TASK-106 | Vulnerable-app benchmark suite in CI (juice-shop + vampi + crapi) | 14 | 🔄 **Scaffolded 2026-05-01** — `scripts/benchmark/run-juice-shop.sh` + `make benchmark` + `.github/workflows/benchmark.yml` (workflow_dispatch) + `docs/benchmarks.md` shipped. CI workflow installs Fendix via `get.fendix.dev/install.sh` (smoke-tests the install pipe too) and uploads `findings.json` + `summary.json` as artifacts. Real numbers land in a follow-up commit after CI runs. vAPI + crapi fixtures intentionally deferred — one fixture is enough to start producing numbers; pattern is copy-paste. |
| TASK-105 | `fendix init` zero-config workflow generator | 14 | ✅ **Shipped 2026-05-01** — new `internal/initcmd` package + `fendix init` cobra command. Detects 7 stacks (Go/Python/Node.js/Ruby/Rust/Java/Kotlin/PHP) by canonical marker file + OpenAPI spec at 14 conventional paths. Writes `.github/workflows/fendix.yml` (embedded copy of `examples/github-actions/fendix-scan.yml` via `go:embed`) + `.fendix-ignore` starter. Refuses to overwrite by default; `--force` overrides; `--print` dry-runs to stdout. Pre-flight clobber check fires before any write so a partial-init state is impossible. 12 unit tests + 3 e2e tests. `.fendix.yaml` deferred to TASK-109. **Frontend sync 2026-05-01**: documented in `app/cli-reference/page.tsx` Commands list + dedicated Init Flags section; advertised in changelog Unreleased entry. Not exposed via backend `LaunchScanSerializer` — `fendix init` is a one-shot bootstrap, not a per-scan orchestration knob (same rationale as `--debug-bundle`). |
