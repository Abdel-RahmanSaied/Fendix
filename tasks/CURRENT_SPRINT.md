...
| --- | --- | --- | --- | --- |
| TASK-119 | Native dep-CVE scanners — govulncheck / pip-audit / npm-audit in `internal/scanner/deps/{govulncheck,pip,npm}/` | 🔄 2/3 sub-deliverables (govulncheck shipped 2026-05-11 commit `5bf874b`; pip-audit shipped 2026-05-11 commit `aade7e1`) | ~2 days remaining | govulncheck ✅ — `golang.org/x/vuln/scan` in-process, call-graph reachability, --no-native-deps escape hatch, Go 1.21→1.22 toolchain bump required (x/tools v0.17.0 incompatibility). pip-audit ✅ — OSV.dev `/v1/query` per pinned `==` dep, 24h cache at `~/.fendix/cache/osv-pypi/`, ranges skipped (same posture as pip-audit binary), 17 unit tests. npm-audit pending: same shape, parses `package-lock.json` / `pnpm-lock.yaml`, OSV.dev for npm ecosystem. |
